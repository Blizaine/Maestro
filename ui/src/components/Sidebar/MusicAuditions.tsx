...
export function MusicAuditions({project, settings, onChange, busy, run}: {
  ...
  return <div className="space-y-3 rounded-lg border border-border p-3">
    ...
    <p className="text-xs text-text-muted">Save, render the same short test song, then resume training. Uses extra GPU time every 200 style steps or 100 sound-adaptation steps, and at the final checkpoint. Preview audio stays with this project.</p>
    {settings.enabled && <fieldset disabled={disabled} className="space-y-3 disabled:opacity-60">
      ...
      }}><option value="">Choose checkpoint…</option>{checkpoints.map((row, index) => <option key={`${row.branch}-${row.step}`} value={index}>{row.branch === 'audio' ? 'Sound adaptation' : 'Style'} · step {row.step}</option>)}</select></label>}
    </fieldset>}
    {!!project.auditions?.length && <div className="space-y-3 border-t border-border pt-3">
      ...
      {project.auditions.map(sample => <div key={sample.id} className="space-y-2 rounded border border-border p-2">
        <p className="text-xs">{sample.branch === 'audio' ? 'Sound adaptation' : 'Style'} · step {sample.step} · {sample.status === 'completed' ? `${sample.seconds?.toFixed(1)}s` : sample.status}</p>
