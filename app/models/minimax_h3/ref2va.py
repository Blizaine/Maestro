...
class MiniMaxH3PreparedReference:
    ...
    audio_intent: str = ""
    image_intent: str = ""
    refmod_latent: torch.Tensor | None = None
...
def prepare_references(
    ...
    *,
    ...
    items = validate_reference_manifest(ordered_items, require_files=True,
        require_visual=not audio_only, allow_empty=audio_only)
    max_duration = num_frames / MINIMAX_H3_FPS
    if audio_only:
        # Audio output may span 45s, but uploaded references still share the
        # original 15s budget. The dialogue runner trims them before this pass.
        max_duration = 15.0
    ...
    for item in items:
        ...
        if item.get("refmod_path"):
            from services.refmod import load_visual_latent
            _info, reference.refmod_latent = load_visual_latent(item["refmod_path"])
            reference.num_latent_frames, reference.latent_height, reference.latent_width = (
                int(n) for n in reference.refmod_latent.shape[2:]
            )
