...
- **Direct generation** is the default: YuE2 makes the recording from your lyrics and style without first writing a score.
- **Melody and chords** optionally plans the tune and its harmony before making the recording.
...
- `model_mode`: **2** direct generation (default), **0** melody+chords, **1** melody only/free accompaniment.
...
class family_handler:
    ...
    @staticmethod
    def get_lora_dir(base_model_type, args=None, lora_root="loras"):
    ...
    def query_model_def(base_model_type, model_def):
        return {
            ...
            "model_modes": {"choices": [("Direct generation", 2), ("Melody and chords", 0), ("Melody only", 1)], "default": 2, "label": "Composition Planning"},
    ...
    def load_model(model_filename, model_type, base_model_type, model_def, dtype=None, VAE_dtype=None, save_quantized=False, profile=0, lm_decoder_engine="legacy", text_encoder_filename=None, **kwargs):
        ...
        acoustic_weights = model_filename[0] if isinstance(model_filename, (list, tuple)) else model_filename
    ...
    def update_default_settings(base_model_type, model_def, ui_defaults):
        ui_defaults.update({"prompt": PROMPT, "alt_prompt": STYLE, "audio_prompt_type": "", "duration_seconds": 120, "video_length": 0, "num_inference_steps": 32, "guidance_scale": 1.0, "temperature": 1.0, "top_k": 100, "top_p": 0.95, "model_mode": 2, "custom_settings": {"abc": ""}, "prompt_enhancer": "", "negative_prompt": "", "repeat_generation": 1, "multi_prompts_gen_type": "FG"})

    @staticmethod
    def validate_generative_prompt(base_model_type, model_def, inputs, one_prompt):
        from .prompting import validate_song_inputs
        return validate_song_inputs(inputs, one_prompt)
