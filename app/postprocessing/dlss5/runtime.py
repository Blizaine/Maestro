...
def unavailable_reason(*, temporal: bool) -> str:
    ...
    if temporal:
        ...
        if not capabilities:
            return "the installed Frame Generation worker did not report its capabilities"
...
class Worker:
    def __init__(self, command: list[str], cwd: Path, abort_callback=None):
        ...
        self._io_deadline = 0.0
        def watch_abort():
            while not self._closed.wait(0.2):
                if self.process.poll() is not None:
                    return
                expired = self._io_deadline and time.monotonic() > self._io_deadline
                if expired or (abort_callback is not None and abort_callback()):
                    if expired:
                        self.logs.append("Native worker timed out while transferring a frame or waiting for a reply.")
                    try:
                        self.process.terminate()
                    except OSError:
                        pass
                    return
        self._abort_thread = threading.Thread(target=watch_abort, daemon=True)
        self._abort_thread.start()

    def write(self, data):
        self._io_deadline = time.monotonic() + 120
        try:
            view = memoryview(data).cast("B")
            sent = 0
            while sent < len(view):
                count = self.process.stdin.write(view[sent:])
                if not count:
                    raise RuntimeError("DLSS worker closed its input pipe")
                sent += count
        except (OSError, RuntimeError) as error:
            self.close(abort=True)
            raise RuntimeError(f"DLSS worker input failed: {error}\n{self.error()}") from error
        finally:
            self._io_deadline = 0.0

    def read_array(self, shape, output=None):
        self._io_deadline = time.monotonic() + 120
        try:
            return _read_array(self.process.stdout, shape, output)
        finally:
            self._io_deadline = 0.0
    ...
    def close(self, *, abort: bool = False):
        ...
        if self._abort_thread is not None:
            self._abort_thread.join(timeout=1)
...
class NeuralRenderingSession(Worker):
    ...
    def __init__(self, width: int, height: int, frames: int, scale: float, intensity: float = 1.0, abort_callback=None, *, host_dir: Path | None = None):
        ...
        if USE_DEPTH_GUIDE:
            self.write(struct.pack("<10I", self.DEPTH_VIDEO_MAGIC, width, height, output_width, output_height, frames, 0, 60, 0, 0))
        else:
            _name, perf_quality = NR_MODES[scale]
            self.write(struct.pack("<14I4f", self.VIDEO_MAGIC, width, height, output_width, output_height, 0, frames, perf_quality, 0, 0, 0, 0, 0, 0, intensity, 1.0, 1.0, -1.0))
    ...
    def process_frame(self, index: int, rgba: np.ndarray, motion: np.ndarray, reset: bool, output: np.ndarray | None = None, *, depth: np.ndarray | None = None) -> np.ndarray:
        assert self.process.stdin is not None and self.process.stdout is not None
        self.write(struct.pack("<4Iq", self.FRAME_MAGIC, index, int(reset), 0, index))
        self.write(memoryview(np.ascontiguousarray(rgba, dtype=np.uint8)).cast("B"))
        self.write(memoryview(np.ascontiguousarray(motion, dtype=np.float16)).cast("B"))
        if USE_DEPTH_GUIDE:
            if depth is None:
                raise ValueError("DLSS Neural Rendering depth mode requires a depth guide")
            self.write(memoryview(np.ascontiguousarray(depth, dtype=np.float32)).cast("B"))
        self.process.stdin.flush()
...
class FrameGenerationSession(Worker):
    ...
    def __init__(self, width: int, height: int, frames: int, generated_count: int, abort_callback=None):
        super().__init__([str(DLSSG_WORKER), "--serve"], DLSSG_DIR, abort_callback)
        self.width, self.height = width, height
        self.generated_count = generated_count
        assert self.process.stdin is not None and self.process.stdout is not None
        self.write(struct.pack("<5I", self.SETUP_MAGIC, width, height, max(1, frames), generated_count))
        self.process.stdin.flush()
    ...
    def process_frame(self, index: int, rgba: np.ndarray, motion: np.ndarray, timestamp: Fraction, reset: bool, output: np.ndarray | None = None) -> int:
        assert self.process.stdin is not None and self.process.stdout is not None
        self.write(struct.pack("<4I2q", self.FRAME_MAGIC, index, int(reset), 0, timestamp.numerator, timestamp.denominator))
        self.write(memoryview(np.ascontiguousarray(rgba, dtype=np.uint8)).cast("B"))
        self.write(memoryview(np.ascontiguousarray(motion, dtype=np.float16)).cast("B"))
        self.process.stdin.flush()
        ...
        for index in range(generated):
            self.read_array((self.height, self.width, 4), output[index])
        return generated
...
def _interpolate(frame_count: int, width: int, height: int, frame_getter, fps: float, scale: int, motion_vector: str, abort_callback=None, progress_callback=None) -> np.ndarray | None:
    ...
    session = FrameGenerationSession(width, height, frame_count, generated_count, abort_callback)
    completed = False
    try:
