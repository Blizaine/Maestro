...
2. Choose **Use a character**, then **Saved character** or **Upload image**.
   ...
   changes under **Appearance**. Use the first source frame or pause the video
   and choose **Use paused frame**. **Preview replacement frame** is optional;
...
3. Start with **480p**. The aspect ratio follows the source. Auto duration follows
   the control video; Time or Window lets you choose another runtime.
4. Choose no input audio, the control video's soundtrack, or custom audio.
   ...
   track when retaining source or custom audio.
